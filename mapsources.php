 ...
 *    modified by Tanner M. Young, 2012 
 *
...
*/
		...
		#
		# nlwiki zoom: compatibility
		# 
		if ( !isset( $attr['scale'] ) && isset( $attr['zoom'] ) ) {
			# Incompatible with {zoom} and {osmzoom}
			$attr['scale'] = pow ( 2, 12 - $attr['zoom'] ) * 100000;
		}
...
/*		if ($bsarticle->getID() == 0) {
		...
		$search = array( 
			...
			'{geocountry}', '{geoa1}',
			'{params}', '{language}',
			'{pagename_gmaps}', '{mapscript}',
      '{textlocation}') ;
		
		# grab global varibles from geohack.php
		global $r_title, $r_pagename, $textLocation;# FIXME operations back in here
		...
		$replace = array(
			...
			$pagename_gmaps,
      '<div id="dynamicmaps"></div><script type="text/javascript">function lMaps(){document.getElementById("dynamicmaps").innerHTML=\'<iframe id="mapFrame" style="width:90%;height:510px;border:0px;margin:auto;padding:0px;display:block" src="http://www.alyoung.com/My_Fathers_Captivity/Map.html?lat=' . $lat['deg'] . '&long=' . $lon['deg'] . '&gZoom=16"></iframe>\';}window.onload=lMaps;</script>',
      $textLocation
		);
		return str_replace( $search, $replace, str_replace('&#123;','{',$bstext) );
	}
}
?>